...
int main() {
    ...
    std::cout << "[+] seed (42 bytes): " << bytes_to_hex(cipher.GetSeed(), SEED_SIZE) << "\n";
    ...
    const uint8_t* payload_ciphertext_ptr = cipher.GetPayloadEncCiphertext();
    size_t payload_ciphertext_len = cipher.GetPayloadEncCiphertextLen();

    if (!cipher.Decrypt(payload_ciphertext_ptr, payload_ciphertext_len, demon_payload)) {
