...
bool AESDecrypt(std::vector<uint8_t>& ciphertext, const uint8_t* key, const uint8_t* iv) {
    ...
    // Faire une copie de l'IV car BCryptDecrypt peut le modifier
    uint8_t iv_copy[16];
    memcpy(iv_copy, iv, 16);

    status = BCryptDecrypt(hKey, (PUCHAR)ciphertext.data(), (ULONG)ciphertext.size(),
                          NULL, (PUCHAR)iv_copy, 16, decrypted.data(), (ULONG)decrypted.size(),
                          &cbDecrypted, 0);
    if (!BCRYPT_SUCCESS(status)) {
    ...
    // Vérification que le padding ne dépasse pas la taille décryptée
    if (pad_len > cbDecrypted) {
        return false;
    }
    ...
    size_t plaintext_size = cbDecrypted - pad_len;
    try {
        ciphertext.clear();
        ciphertext.resize(plaintext_size);
        std::memcpy(ciphertext.data(), decrypted.data(), plaintext_size);
    } catch (const std::bad_alloc& e) {
        std::cerr << "[-] AESDecrypt bad_alloc: plaintext_size=" << plaintext_size << "\n";
        return false;
    }
